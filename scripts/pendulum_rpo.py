...
parser.add_argument('--rpo-alpha', type=float, default=0.5, help='RPO noise scale for policy update')  # Добавлено
...
class RolloutBuffer:
    def __init__(self, max_steps):
        ...
        self.log_probs = []
        self.dones = []
        self.values = []  # Добавлено для хранения значений критика
    ...
    def get_data(self):
        return {
            ...
            "dones": self.dones,
            "values": torch.FloatTensor(self.values).to(device),  # Добавлено
        }
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=64):
        ...
        self.rpo_alpha = args.rpo_alpha  # Добавлено для RPO
    ...
    def get_action_and_value(self, state, action=None):
        ...
        if action is None:
            action = probs.sample()
        else:  # RPO: добавление стохастичности
...
def update_ppo(states, actions, log_probs_old, returns, advantages):
    for _ in range(args.num_epochs):
        ...
        for start in range(0, len(states), args.sub_batch_size):
            ...
            action, log_prob, entropy = actor.get_action_and_value(state_batch, action_batch)
...
if __name__ == '__main__':
    ...
    plot_timesteps_vs_rewards(timesteps, rewards, "results/timesteps_vs_rewards_rpo_seed_666.png")
    env.close()